...
// MOCK: Chỉ gọi data tĩnh cho Captcha
const fetchCaptcha = async () => {
  isLoading.value = true;
  try {
    // const res = await axiosClient.get("/captcha/generate");
    const mockRes = {
      question: "Chọn vật phẩm: Thanh Long Kiếm",
      options: [
        {
          itemId: 1,
          imageUrl: "https://placehold.co/64x64/607d8b/fff?text=Kiếm",
        },
        {
          itemId: 2,
          imageUrl: "https://placehold.co/64x64/4e342e/fff?text=Giáp",
        },
        {
          itemId: 3,
          imageUrl: "https://placehold.co/64x64/263238/fff?text=Mũ",
        },
        {
          itemId: 4,
          imageUrl: "https://placehold.co/64x64/8d6e63/fff?text=Giày",
        },
      ],
    };
    question.value = mockRes.question;
    options.value = mockRes.options;
  } catch (e) {
...
const submitAnswer = async (itemId) => {
  try {
    ...
    // MOCK: Giả lập kết quả (Thành công nếu chọn ID 1)
    if (itemId === 1) {
