...
const currentSkinId = computed(
  () => authStore.user?.avatarUrl || "skin_langkhach"
);
const selectedSkinId = ref("skin_langkhach");
...
const saveSkin = async () => {
  try {
    // MOCK: Giả lập lưu skin
    await axiosClient.put("/user/update", { avatarUrl: selectedSkinId.value });
    authStore.user.avatarUrl = selectedSkinId.value; // Cập nhật local
...
const renameChar = async () => {
  ...
  try {
    // MOCK: Giả lập đổi tên
    await axiosClient.post("/character/rename", { name: newName.value });
    charStore.character.name = newName.value; // Cập nhật local
    newName.value = "";
...
onMounted(() => {
  ...
  animInterval = setInterval(() => {
    // Luân phiên 3 trạng thái cho Preview
