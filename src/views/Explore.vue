        ...
        <div class="game-board">
          <div class="status-header">
            <div class="level-badge">
              <span>Lv.{{ charStore.character?.lv || 1 }}</span>
            </div>
                  ...
                  <div class="progress-bg">
                    ...
                    <span class="stat-text"
                      >{{ charStore.character?.energy || 0 }}/{{
                        charStore.character?.maxEnergy || 0
                      }}</span
                    >
                  </div>
                </div>
          ...
          <div class="action-panel">
            <template v-if="!isEncounter">
              <button
                ...
                :disabled="isMoving || (charStore.character?.energy || 0) < 1"
              >
...
const imgPlayer = computed(() => {
  ...
  // MOCK: Giả lập chuyển động và tấn công
  if (isMoving.value) return skin.sprites.run;
  if (isEncounter.value) return skin.sprites.attack;
...
const startExploration = () => {
  ...
  if ((charStore.character?.energy || 0) < 1) {
    addLog("<span style='color:red'>HẾT NỘI LỰC! Về làng nghỉ ngơi.</span>");
    return;
  }
...
const handleResult = async () => {
  ...
  try {
    ...
    if (res.type === "GOLD") {
      ...
      battleStore.setEncounter(rndEnemy); // Lưu thông tin quái vật cho trận chiến
...
onMounted(() => {
  ...
  // Khởi động chuyển động nhẹ nhàng ngay từ đầu
  startMovingJS();
});
