        ...
        <form @submit.prevent="handleLogin" class="auth-form">
          <div class="input-group">
            ...
            <input
              v-model="username"
              type="text"
              placeholder="Danh xưng / Tài khoản (Dùng 'LunNu' hoặc 'GiamCam')"
              required
              class="ink-input"
            />
          </div>
...
const username = ref("LunNu"); // MOCK: Tên mặc định
const password = ref("123456"); // MOCK: Mật khẩu mặc định
...
const handleLogin = async () => {
  try {
    ...
    if (
      ...
      authStore.error = null; // Xóa lỗi thường
    } else {
      authStore.error =
        err.response?.data?.error || "Thông tin không chính xác";
    }
  } finally {
